...
export default function RatingMeter({ ...props }) {
    ...
    const RatingStar = ({ ...props }) => {
        if (props.index === null) {
            return (
                <span className="cust-grid-stack shrink-0">
                    <LuStarOff className="opacity-30"/>
                </span>
            )
        ...
        if (r_floored - 0.5 > props.index) {
            return (
                <span className="cust-grid-stack shrink-0">
                    <LuStar className="opacity-30"/>
                    <LuStar className="fill-black" />
                </span>
            )
        ...
        if (r_floored > props.index) {
            return (
                <span className="cust-grid-stack shrink-0">
                    <LuStar className="opacity-30"/>
                    <LuStarHalf className="fill-black" />
                </span>
            )
        }
        return <span className="cust-grid-stack shrink-0"><LuStar className="opacity-30"/></span>
    }


    return (
        <div className="flex gap-2 items-center">
            <span className='flex gap-1 bg-fit-lte w-fit py-0.5 px-2 rounded-4xl items-center'>
